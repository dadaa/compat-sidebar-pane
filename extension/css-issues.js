class CSSCompatibility {
  ...
  async getCurrentDocumentIssues(targetBrowsers) {
    ...
    await browser.tabs.executeScript(tabId, { file: "css-content.js",
                                              runAt: "document_idle" });
    ...
    for (;;) {
      ...
      if (chunk.atrule) {
        ...
        if (isInCSSDeclarationBlock) {
          ...
          if (!this._mdnBrowserCompat.hasProperty(property)) {
  ...
  _analyzeProperty(property, issueMap, targetBrowsers) {
    ...
    for (const targetBrowser of targetBrowsers) {
      const support = this._mdnBrowserCompat.getPropertyState(property,
                                                              targetBrowser.name,
  ...
  _analyzePropertyValue(property, value, issueMap, targetBrowsers) {
    ...
    const unsupportedBrowsers = [];
    for (const targetBrowser of targetBrowsers) {
      const support = this._mdnBrowserCompat.getPropertyValueState(property,
                                                                   value,
                                                                   targetBrowser.name,
                                                                   targetBrowser.version);
